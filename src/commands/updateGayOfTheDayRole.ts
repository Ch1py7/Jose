...
import { DAY, ROLE_NAME } from '@/utils/constants'
import type { Client, Guild, Role } from 'discord.js'

export const updateGayOfTheDayRole = async (guild: Guild, newUserId: string): Promise<void> => {
	const role: Role | undefined = guild.roles.cache.find((r) => r.name === ROLE_NAME)
	if (!role) return
		...
		try {
			...
			if (!error) {
				data.forEach(async ({ id }) => {
					await supabase.from('gay_role_assignments').update({ isGay: false }).eq('id', id)
				})
			}
...
export const reassignGayOfTheDayRole = async (
	client: Client,
	userId: string,
	guildId: string
): Promise<void> => {
